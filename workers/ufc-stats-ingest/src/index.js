...
async function runIngest(env, { invoked = 'cron', cron = null, skipEspn = false, onlyBout = null, force = false } = {}) {
  ...
  try {
    ...
    if (!sourceReason && fetcher.subrequests > 0) await putState(env, STATE.health, { status: 'ok', at: nowIso(), telemetry: fetcher.telemetry() });
...
/* The lane (docs: supabase/migrations/20260910160000_ufc_round_stat_queue.sql).
 *
