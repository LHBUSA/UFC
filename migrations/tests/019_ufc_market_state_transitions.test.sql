...
-- ================= behaviour proof (runs inside BEGIN ... ROLLBACK) =======
do $test$
declare
  n int;
  ok boolean;
...
begin
  -- 1. a transition can be recorded
  insert into public.ufc_market_state_transitions
    (espn_competition_id, kind, round, espn_status, observed_at, provenance)
  values ('999999', 'round_end', 1, 'STATUS_END_OF_ROUND', '2026-09-19T23:05:00Z', 'test');
  select count(*) into n from public.ufc_market_state_transitions where espn_competition_id='999999';
  assert n = 1, 'insert failed';
  select observed_at into first_ts from public.ufc_market_state_transitions where espn_competition_id='999999';

  -- 2. FIRST-SEEN-WINS: a later sighting of the same state is a no-op
  insert into public.ufc_market_state_transitions
    (espn_competition_id, kind, round, espn_status, observed_at, provenance)
  values ('999999', 'round_end', 1, 'STATUS_END_OF_ROUND', '2026-09-19T23:09:00Z', 'test later')
  on conflict (espn_competition_id, kind, round) do nothing;
  select count(*) into n from public.ufc_market_state_transitions where espn_competition_id='999999';
  assert n = 1, 'duplicate state created a second row';
  select (observed_at = first_ts) into ok from public.ufc_market_state_transitions where espn_competition_id='999999';
  assert ok, 'a later sighting overwrote the first-seen timestamp';

  -- 3. APPEND-ONLY: update refused
  begin
    update public.ufc_market_state_transitions set observed_at = now() where espn_competition_id='999999';
    assert false, 'UPDATE was permitted on an append-only table';
  exception when others then null;
  end;

  -- 4. APPEND-ONLY: delete refused
  begin
    delete from public.ufc_market_state_transitions where espn_competition_id='999999';
    assert false, 'DELETE was permitted on an append-only table';
  exception when others then null;
  end;
  select count(*) into n from public.ufc_market_state_transitions where espn_competition_id='999999';
  assert n = 1, 'row disappeared despite append-only';

  -- 5. a round_end must carry a round; in_progress must not
  begin
    insert into public.ufc_market_state_transitions (espn_competition_id, kind, espn_status, provenance)
    values ('888', 'round_end', 'STATUS_END_OF_ROUND', 'test');
    assert false, 'round_end without a round was permitted';
  exception when others then null;
  end;
  begin
    insert into public.ufc_market_state_transitions (espn_competition_id, kind, round, espn_status, provenance)
    values ('888', 'in_progress', 2, 'STATUS_IN_PROGRESS_2', 'test');
    assert false, 'in_progress carrying a round was permitted';
  exception when others then null;
  end;

  -- 6. existing market data is untouched by any of this
...
select
  (select count(*) from public.ufc_market_state_transitions) as transitions_in_txn,
  (select count(*) from public.ufc_market_observations) as observations_untouched;
