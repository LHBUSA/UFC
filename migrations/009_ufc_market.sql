...
create table if not exists public.ufc_market_observations (
  ...
  constraint ufc_market_obs_unique
...
comment on constraint ufc_market_obs_unique on public.ufc_market_observations is
  'Idempotency: re-ingesting an unchanged price is rejected by the database, a changed price is new history. Named explicitly rather than left to the auto-generated identifier, which Postgres truncates at 63 characters.';
