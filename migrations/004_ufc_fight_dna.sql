...
create table if not exists public.ufc_fighter_bout_features (
  ...
  stance_context text check (stance_context is null or stance_context in ('same','open','switch_involved','unknown')),
...
-- Licensed or independently verified SOURCE FACTS. One row may describe a
-- whole fight or one round. A surrogate PK is required because fight-scope
-- rows intentionally have round = null; uniqueness is enforced by partial
-- indexes instead of a nullable composite primary key.
...
create unique index if not exists ufc_bout_position_stats_fight_uniq
  on public.ufc_bout_position_stats (bout_id, fighter_id, source)
  where scope = 'fight';
create unique index if not exists ufc_bout_position_stats_round_uniq
  on public.ufc_bout_position_stats (bout_id, fighter_id, source, round)
  where scope = 'round';
