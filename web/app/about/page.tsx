  ...
  return (
    <div className="wrap page narrow">
      ...
      <div className="prose">
        ...
        <p>Schedules, results and fighter identity come from ESPN's public MMA data. Round-by-round striking and grappling statistics are UFC's official fight statistics, read from UFC Stats or, when UFC Stats cannot be read, from the official statistics feed behind UFC.com event pages. Official rankings are captured from UFC.com as dated snapshots. Every stored row carries its source URL and capture time. Where two sources disagree, we show the disagreement rather than pick a side silently.</p>
        ...
        <p><strong>LIVE</strong> is genuine current provider data. <strong>MODEL</strong> is PropBetEdge model output. <strong>UNAVAILABLE</strong> means a required input is missing. We never relabel one as another, and we never display a pick, edge or probability the model did not produce. <Link href="/algo">PBE Algo</Link> calls are shown to UFC Pro members only for bouts the model is eligible to call, locked before the fight and graded after it; an ineligible bout shows NO MODEL CALL with its reason, and the public record starts at the first lock rather than borrowing from the backtest.</p>
        <h2 id="images">Images</h2>
        <p>Our stored fighter portraits are Creative Commons or public-domain photographs from Wikimedia Commons, matched to the fighter through Wikidata, stored with the author, licence and source, and credited wherever they appear. Where we have no licensed portrait, fighter surfaces may show an externally hosted, display-only headshot, which is not part of the redistributable PropBetEdge media catalog; where none is available we show a branded placeholder. Official fight videos are embedded from the publisher&apos;s own YouTube channel, and their preview image is the thumbnail YouTube provides for that video. We never generate a real fighter&apos;s likeness with AI.</p>
