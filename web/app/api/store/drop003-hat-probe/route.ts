...
export async function GET() {
  ...
  const hits = catalog.filter((p) => /dad hat|embroidered.*(cap|hat)|snapback|trucker cap/i.test(`${p.title} ${p.model || ""} ${p.type || ""}`)).slice(0, 20);
  const rows = [];
  for (const p of hits) {
    ...
    rows.push({
      ...
      model: p.model,
      type: p.type,
      variants: variants.filter((v) => String(v.color || "").toLowerCase() === "black").map((v) => ({ id: v.id, size: v.size, color: v.color, name: v.name, in_stock: v.inStock, price: v.price })),
      print_areas: areas,
    });
  }
