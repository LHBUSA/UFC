 ...
 * before the response starts streaming: a missing voice profile is a real HTTP 404
...
export default async function Gate({ children, params }: { children: React.ReactNode; params: Promise<{ key: string }> }) {
  const { key: slug } = await params;
  if (!getVoice(slug)) notFound();
  return children;
}
