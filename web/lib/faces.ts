...
import "server-only";
import { getFightersByIds, type Article, type Fighter, type PortraitSet } from "@/lib/db";
...
export async function storyMedia(articles: Article[]): Promise<StoryMedia> {
  ...
  const [fighters, imgs, heroPairs] = await Promise.all([
    ...
    resolveFighterPortraits(ids, { surface: "high_visibility" }),
    Promise.all(heroIds.map(async (id) => [id, await resolveArticleHero(id, { surface: "high_visibility" })] as const)),
  ]);
