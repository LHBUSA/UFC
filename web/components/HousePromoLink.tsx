...
export function HousePromoLink({ href, placement, dest, slug, track, className, newTab = false, children }: {
  href: string; placement: "inline" | "end"; dest: string; slug: string; track: boolean;
  className?: string; newTab?: boolean; children: React.ReactNode;
}) {
  const onClick = () => {
    if (!track) return;
    try {
      const body = JSON.stringify({ placement, dest, slug });
