        ...
        <div className="voices">
          {VOICES.map((v) => (
            <article className="voice" key={v.key}>
              ...
              <Link className="voice-media" href={`/voices/${v.key}`} aria-label={`Read ${v.name} profile`}>
              ...
              <div className="voice-body">
                ...
                <h3><Link href={`/voices/${v.key}`}>{v.name}</Link></h3>
                <p>{v.descriptor}</p>
                <div className="voice-actions">
                  <Link className="voice-cta" href={`/voices/${v.key}`}>
                  ...
                  <a className="voice-cta voice-cta-muted" href={v.href} target="_blank" rel="noopener nofollow" aria-label={`${v.cta} — opens ${v.destination} in a new tab`}>
                    <span>{v.cta}</span>
                  </a>
                </div>
