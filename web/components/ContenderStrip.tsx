...
import { contenderSeason, contenderWeek } from "@/lib/contender";
...
export function ContenderStrip({ next, last, mains, counts, freshness }: {
  ...
  const row = (e: Event, kind: "next" | "last") => {
    ...
    const season = contenderSeason(e.name, e.event_date), week = contenderWeek(e.name);
    return (
      <Link href={`/events/${eventSlug(e)}`} className={`dwcs-strip-card ${kind}`} key={e.id}>
        <div className="dwcs-strip-kicker">{kind === "next" ? (d === 0 ? "Tonight" : d != null && d <= 6 ? "This week" : "Next episode") : "Last result"}{season ? ` · Season ${season}` : ""}{week ? ` · Week ${week}` : ""}</div>
