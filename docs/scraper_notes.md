...
| Fighter page | record, physicals, career stats, fight history | **Matches, one caveat.** `h2 .b-content__title-highlight` (name) + `.b-content__title-record` (`Record: 23-4-0`); `p.b-content__Nickname`; box items `Height: 5' 9"`, `Weight: 135 lbs.`, `Reach: 70"`, `STANCE: Switch`, `DOB: Nov 30, 1995` (abbreviated month); career `SLpM, Str. Acc., SApM, Str. Def` (no trailing period), `TD Avg., TD Acc., TD Def., Sub. Avg.`; fighters with no UFC stats print `0.00`/`0%`, not `--`. History table `.b-fight-details__table_type_event-details`, headers `W/L, Fighter, Kd, Str, Td, Sub, Event, Method, Round, Time`, `data-link` per row. **Caveat: the history includes non-UFC promotions** (Strikeforce rows on Tom Aaron's page), so the completeness cross-check must intersect `history_fight_ids` with `ufc_bouts.ufcstats_id`, not compare counts. Captures: 18d01f7f8338ae72 (2026-02-13), 93fe7332d16c6ad9 (2026-02-18). |
| Upcoming list | same shape as completed | **Unverified.** The only recent capture (2026-08-01) is the JS interstitial. The Worker takes upcoming cards from ESPN, and the backfill treats a missing list capture as a counted gap, so nothing depends on it. |
