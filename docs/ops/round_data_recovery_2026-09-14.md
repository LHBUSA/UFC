...
Recovered on Worker v0.8.5 (`62bf2f9a`) through the normal lane (`POST /admin/run?espn=false&bout=ac23cd35-fa8a-4517-8615-4c682a4d77af&force=true`): 6 round rows (rounds 1-3, both corners), queue `written`, identity evidence records the override and both DOBs. Round rows 42,108 -> 42,114 (+6 exactly); event, bout, result, fighter (Kwon DOB still 1995-06-24) and alias rows byte-identical on every card. **Contender Series Season 10: 25/25 bouts, 102 round rows.** `/health`: lane ok, 0 gaps, `round_archive_review` empty, UFC Stats blocked and disabled. The fight page renders Round-by-Round Analysis R1-R3 at 1440 and 390 with no overflow.
