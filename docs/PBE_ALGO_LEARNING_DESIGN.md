...
| Challenger spec hash | `specCanonical` under the fixed label `pbe-fight-model-challenger`, coefficients and scale stored at 10 dp | depends only on coefficients, scale, λ; promotion re-hashes under the new version name. The same fit in Node and workerd agrees only to ~2e-15, so raw values do not hash identically across runtimes; at 10 dp all 33 agree. The first challenger (run `e250a579`, code `a3feed0`) predates this and stores raw values; it verifies against its own stored spec and is superseded by the next run with new data |
