...
//
// Classification needs TWO facts, and for a while it made do with one.
//
//   The episode number says the bout was broadcast inside the season. That is
//   where it sat in the show. It is NOT a claim about sanctioning — a bout can
//   air in an episode and still have been contested on a card, and treating
//   the episode as proof of an exhibition was asserting the conclusion.
//
//   The second fact is that no professional bout of that description exists.
//   An absence only carries that weight in a year our own records cover, which
//   is why record_completeness.json is consulted rather than assumed. In a year
//   where every event holds bouts and every bout holds a result, a sanctioned
//   bout would be in there. For 2024 and 2026 it might not be, so those seasons'
//   bouts stay unverified and record why.
//
// A bout is never marked 'professional' here; the finale bouts are verified
// against our own result rows separately. Anything without both legs is
// 'unverified' with no source, which is what it is.
...
/* Per-year completeness of our own fight records. An episode number says when
 * a bout was broadcast; it does not say the bout was unsanctioned. The second
 * fact needed is that no professional bout of the same description exists —
 * and an absence only carries that weight in a year our records actually
 * cover. Generated by scripts/tuf/record_completeness.mjs and cited, not
 * assumed. */
const COMPLETENESS_FILE = path.join(ROOT, 'web', 'data', 'tuf', 'record_completeness.json');
const COMPLETENESS = fs.existsSync(COMPLETENESS_FILE)
  ? JSON.parse(fs.readFileSync(COMPLETENESS_FILE, 'utf8'))
  : { years: {} };

/**
 * How a bout aired in a numbered episode should be classified.
 *
 * Two legs, and both have to hold. The episode places the bout inside the
 * season's production. The completeness of our records for that year is what
 * turns "we did not find a professional bout" into "there was none" — in a
 * year where every event carries bouts and every bout carries a result, a
 * sanctioned bout that happened would be there.
 *
 * Where the second leg is missing the bout stays unverified and says why.
 * Season 34 is airing now and 2026 is half-loaded; that season gets no
 * classification it has not earned.
 */
function classifyEpisodeBout(episode, year) {
  const rec = COMPLETENESS.years?.[String(year)];
  if (!episode) return { classification: 'unverified', classification_source: null };
  if (!rec || !rec.complete) {
    return {
      classification: 'unverified',
      classification_source: null,
      classification_note: `Broadcast in episode ${episode}. Left unverified because our fight records for ${year} are not complete${rec ? ` (${rec.events_without_bouts} event(s) without bouts, ${rec.bouts - rec.bouts_with_result} bout(s) without a result)` : ''}, so the absence of a professional bout for it is a gap in our data rather than a fact about the fight.`,
    };
  }
  return {
    classification: 'exhibition',
    classification_source:
      `broadcast in episode ${episode} of the season, and absent from our fight records for ${year}, ` +
      `which are complete for that year — ${rec.events} events, ${rec.bouts} bouts, every one carrying a result — ` +
      'so a bout contested on a sanctioned card that year would be in them',
  };
}

/**
 * The same two legs, for a season scored as a series rather than bracketed.
 *
 * The placement leg is the source's own results table, which records these as
 * bouts of the gym-versus-gym competition. The second leg is unchanged: our
 * records for the year are complete, so a sanctioned bout would be in them.
 */
function classifySeriesBout(year) {
  const rec = COMPLETENESS.years?.[String(year)];
  if (!rec || !rec.complete) {
    return {
      classification: 'unverified',
      classification_source: null,
      classification_note: `Recorded by the season source as contested inside the competition, but our fight records for ${year} are not complete, so the absence of a professional bout for it proves nothing.`,
    };
  }
  return {
    classification: 'exhibition',
    classification_source:
      'contested inside the competition as part of the scored gym-versus-gym series, ' +
      `and absent from our fight records for ${year}, which are complete for that year — ` +
      `${rec.events} events, ${rec.bouts} bouts, every one carrying a result — ` +
      'so a bout contested on a sanctioned card that year would be in them',
  };
}
 ...
 *
 * A wildcard bout, a second chance, an injury replacement and a bye all put a
 * fighter into a round he did not win his way into. A bracket template shows
 * none of that — it shows a tree — so the shape check called every one an
 * inconsistency, and thirteen seasons sat flagged for having the formats this
 * show is known for. Tecia Torres losing and being given a second chance is
 * not a hole in season 20; it is season 20.
 *
 * Read from the episode prose, which states them plainly, each keeping the
 * sentence it came from. A fighter is attached only when the sentence names
 * someone the season already knows about, so nothing is inferred from prose
 * that names nobody. */
const EXCEPTION_KINDS = [
  ['wildcard', /\bwild ?card\b/i],
  ['second chance', /\bsecond chance\b/i],
  ['replacement', /\breplac(?:e[ds]?|ing|ement)\b/i],
  ['withdrawal', /\b(?:withdrew|withdrawn|forced out|pulled out|cannot compete|could not compete)\b/i],
  ['bye', /\b(?:bye|advanced directly)\b/i],
  ['injury', /\binjur(?:y|ed|ies)\b/i],
];

function parseFormatExceptions(wt, knownNames) {
  ...
  for (const raw of section.split(/\n/)) {
    ...
    if (!text) continue;
    const kinds = EXCEPTION_KINDS.filter(([, re]) => re.test(text)).map(([k]) => k);
    if (!kinds.length) continue;
    const folded = fold(text);
    const named = known.filter((n) => {
      const surname = fold(n).split(' ').pop();
      return surname.length > 2 && new RegExp(`\\b${surname}\\b`).test(folded);
    });
    if (!named.length) continue;
    out.push({ kinds, fighters: named, episode, detail: text.slice(0, 400) });
  }
  return out;
}
 ...
 */
function withIntegrity(previous, stage, weightClass, conflicts, exceptions = []) {
  ...
  let unexplained = (!isElimination && bracketPrevious.length)
    ? [...new Set(stage.bouts.flatMap((b) => [b.a, b.b]))].filter((n) => !advanced.has(fold(n)))
    : [];

  /* An entrant the season's own prose accounts for is not an anomaly. A
   * wildcard pick, a second chance, a replacement for someone injured, a bye
   * when an opponent missed weight — the source says so in words, and a
   * bracket drawn as a tree simply cannot show it. Those names are removed
   * from the flag and recorded as explained, with the sentence that explains
   * them. Whatever the prose does NOT account for stays flagged. */
  const explainedBy = new Map();
  if (unexplained.length && exceptions.length) {
    unexplained = unexplained.filter((n) => {
      const hit = exceptions.find((x) => x.fighters.some((f) => fold(f) === fold(n)));
      if (hit) { explainedBy.set(n, hit); return false; }
      return true;
    });
  }

  const explained = [...explainedBy.entries()].map(([name, x]) => ({
    fighter: name,
    kinds: x.kinds,
    episode: x.episode,
    evidence: x.detail,
  ...
  if (!notes.length) return explained.length ? { ...stage, format_exceptions: explained } : stage;
  ...
  return {
    ...stage,
    ...(explained.length ? { format_exceptions: explained } : {}),
    status: 'unverified',
    note: `Partial. ${notes.join('; ')}. See conflicts.`,
  };
}

function eliminationStage(bouts, year) {
  return {
    stage: 'elimination',
    ...
    bouts: bouts.map((e) => ({
      ...
      time: e.time,
      episode: e.episode,
      ...classifyEpisodeBout(e.episode, year),
    })),
  };
}
...
const main = async () => {
  ...
  /* Every fighter the season knows about, used to attach a documented format
   * exception to a person rather than to a sentence. */
  const knownNames = new Set([
    ...teams.flatMap((t) => t.roster.map((r) => r.name)),
    ...brackets.flatMap((b) => (b.bouts || []).flatMap((x) => [x.a, x.b])),
    ...episodes.flatMap((e) => [e.winner, e.loser]),
  ].filter(Boolean));
  const formatExceptions = parseFormatExceptions(wt, knownNames);
  ...
  for (const b of brackets) {
    ...
    for (const st of b.stages) {
      ...
      for (const raw of st.bouts) {
        ...
        const classified = !isFinal && ep && ep.episode
          ? classifyEpisodeBout(ep.episode, row.year)
    ...
    if (leftovers.length) {
      ...
      stages.unshift(eliminationStage(leftovers, row.year));
    }
     ...
     * out of place. */
    const checked = [];
    for (const st of stages) checked.push(withIntegrity(checked, st, b.weightClass, conflicts, formatExceptions));
  ...
  const detail = {
    ...
    ...(teams.length ? { teams } : {}),
    ...(formatExceptions.length ? { format_exceptions: formatExceptions } : {}),
