...
ESPN is the primary source for events/bouts/fighters (Worker). This script
therefore LINKS UFC Stats ids onto rows that already exist and only inserts
what ESPN never saw (everything before the ESPN ingest started):
  ...
  * fighter: page parsed (name + DOB + record) -> shared/alias_resolver.
             ...
             Fighter-list rows are inserted only when the name has no
             ufcstats-less row in the archive (those resolve at page level).
...
fight page -> both fighter pages (fetched BEFORE the bout row so identity is
resolved with DOB, never by name alone).

On any SchemaAssertionError: stop, print the URL, post to Discord, exit 2.
On AccessGateError (JS challenge, live mode only): stop, exit 3.
"""
...
class Backfill:
    def __init__(self, args):
        ...
        self.fighters: dict[str, dict] = {}          # uuid -> row
    ...
    def finish_run(self, status: str):
        ...
        self.db.patch("ufc_ingest_runs", f"id=eq.{self.run_id}", {
            ...
            "notes": {"phase": self.args.phase, "source": self.args.source, "since": self.args.since, "limit": self.args.limit,
                      "counters": c, "log": str(self.log.path), "wayback_missing": self.missing[:2000]},
        })
    ...
    def load_existing(self):
        ...
        self.results = {r["bout_id"] for r in self.db.select_all("ufc_bout_results", "bout_id")}
    ...
    def phase_fighters(self):
        ...
        for ch in "abcdefghijklmnopqrstuvwxyz":
            ...
            for f in parsers.parse_fighter_list(html, url):
                ...
                # a same-name row without a ufcstats_id (ESPN-first) must be resolved at page level with DOB, not here
                if any(self.fighters[i].get("ufcstats_id") is None for i in self.resolver.by_norm.get(normalize(name), ())):
                    self.log.bump("fighters_deferred_to_page")
                    continue
    ...
    def _ingest_fighter(self, fighter_id: str) -> dict | None:
        """Fetch + parse the fighter page; link to an existing row via the resolver or insert. Returns the row."""
        ...
        if self.args.dry_run:
            ...
            row = target or {"id": f"dry-{fighter_id}", "ufcstats_id": fighter_id, "name": p["name"], "dob": p["dob"], "record_w": p["record_w"], "record_l": p["record_l"], "record_d": p["record_d"]}
            if not target:
                self._register_fighter(row)
            else:
    ...
    def _ingest_event(self, ev: dict):
        ...
        bouts = page["bouts"]
        if all(b.get("ufcstats_id") in self.bout_by_ufcstats for b in bouts) and not self.args.force \
                and all(self.bout_by_ufcstats[b["ufcstats_id"]]["id"] in self.results for b in bouts):
            self.log.event("event_skip", id=ev["ufcstats_id"], reason="all bouts and results present")
            return
    ...
    def _ingest_fight(self, fight_id: str):
        ...
        if bout is None:
            ...
            return
        if bout["id"] in self.results and not self.args.force:
            return
        ...
        sc = f["scorecards"] or []
        self.db.upsert("ufc_bout_results", [{
            ...
            "referee": f["referee"], "judge_1": sc[0]["judge"] if len(sc) > 0 else None, "judge_2": sc[1]["judge"] if len(sc) > 1 else None,
            "judge_3": sc[2]["judge"] if len(sc) > 2 else None, "scorecards": f["scorecards"], "finish_detail": f["finish_detail"],
            "has_stats": f["has_stats"], "result_source": "ufcstats", "source_url": url, "captured_at": now_iso(),
        }], on_conflict="bout_id")
        ...
        try:
            ...
            if code == 0 and not self.args.dry_run:
                ...
                discord(self.cfg, f"{WORKER} ok: events_new={c.get('events_new', 0)} bouts_new={c.get('bouts_new', 0)} "
                                  f"fighters_touched={c.get('fighters_touched', 0)} rounds={c.get('round_rows', 0)} "
                                  f"no_stats={c.get('results_without_stats', 0)} missing={sum(v for k, v in c.items() if k.startswith('wayback_missing'))}")
        return code
