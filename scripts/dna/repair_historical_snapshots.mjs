...
// It patches the committed builder (scripts/dna/build_fight_dna.mjs) into a
// temporary history-repair build that regenerates every historical pre-fight
// snapshot (one per fighter per day-after-bout) from backfilled evidence.
//
// The canonical builder now owns the structural fixes this script used to
// patch in at runtime (no row caps, explicit ORDER BY, EXCLUSIVE as-of cutoff
// as of build_fight_dna@v1.2). patchBuilderSource() therefore ASSERTS those
// forms are present instead of rewriting them, and only adds the historical
// snapshot loop, the per-snapshot metric clock and the proof payload.
//
// Usage:
...
//
// Importing this module runs nothing: patchBuilderSource(src) is a pure
// function over the builder source text (used by build_fight_dna.test.mjs to
// prove the anchors still resolve). The CLI lives behind the isCli guard.
...
/* The canonical builder must already carry these forms; the repair refuses to
 * run against a builder that regressed rather than silently re-patching it. */
function assertOnce(src, needle, label) {
  const first = src.indexOf(needle);
  if (first < 0) throw new Error(`repair precondition missing in builder: ${label}`);
  if (src.indexOf(needle, first + needle.length) >= 0) throw new Error(`repair precondition duplicated in builder: ${label}`);
}
function assertAbsent(src, needle, label) {
  if (src.indexOf(needle) >= 0) throw new Error(`repair precondition violated in builder: ${label} (${needle})`);
}

/**
 * Pure: builder source text in, history-repair builder source text out.
 * Throws when an anchor no longer resolves against the builder, so a drifted
 * builder fails loudly before anything is spawned.
 */
export function patchBuilderSource(source) {
  /* Anchors are LF; a CRLF checkout (core.autocrlf on Windows) must still match. */
  let src = String(source).replace(/\r\n/g, '\n');

  /* --- preconditions: structural fixes now live in the canonical builder --- */
  assertOnce(src, "selectAll('ufc_events', `select=id,name,event_date&event_date=lt.${AS_OF}&order=event_date.asc,id.asc`)", 'exclusive event cutoff (event_date < AS_OF)');
  assertAbsent(src, 'event_date=lte.', 'inclusive event cutoff must not be present');
  assertOnce(src, 'return boutContributes(e?.event_date, AS_OF) && fighterMap.has(b.fighter_a_id) && fighterMap.has(b.fighter_b_id) && (!ONLY_FIGHTER || b.fighter_a_id === ONLY_FIGHTER || b.fighter_b_id === ONLY_FIGHTER);', 'exclusive relevant-bout cutoff via boutContributes');
  assertAbsent(src, 'e.event_date <= AS_OF', 'inclusive relevant-bout cutoff must not be present');
  assertOnce(src, "&status=eq.complete&model_scope=eq.true&order=id')", 'all complete model-scope bouts, ordered, uncapped');
  assertOnce(src, "stats_source_url,stats_captured_at&order=bout_id')", 'all results, ordered, uncapped');
  assertOnce(src, "source_url,captured_at&order=bout_id,fighter_id,round`)", 'all round stats, ordered, uncapped');
  assertAbsent(src, '&limit=', 'row caps in selectAll queries');
...
`;
  ...
  src = replaceOnce(
    ...
    "const summary = { ok: true, build_run_id: runId, as_of: AS_OF, input_counts: sourceCounts.input_counts, ...output };",
    'proof payload',
  );
  /* The builder only runs its CLI when argv[1] is its own path; the temp copy
   * has a different basename, so the guard must be retargeted or the spawned
   * child does nothing and emits no proof. */
  src = replaceOnce(
    src,
    "&& process.argv?.[1]?.replace(/\\\\/g, '/').endsWith('scripts/dna/build_fight_dna.mjs');",
    "&& process.argv?.[1]?.replace(/\\\\/g, '/').endsWith('scripts/dna/.history-repair-builder.tmp.mjs');",
    'temp builder CLI guard',
  );
...
/* CLI ONLY. Importing this module must never read env, count rows or spawn. */
const isCli = typeof process !== 'undefined'
  && process.argv?.[1]?.replace(/\\/g, '/').endsWith('scripts/dna/repair_historical_snapshots.mjs');
if (isCli) {
  ...
  const exactCount = async (table, column, filter = '') => {
  ...
  const main = async () => {
    ...
    fs.writeFileSync(TEMP, patchBuilderSource(fs.readFileSync(SOURCE, 'utf8')), 'utf8');
    try {
