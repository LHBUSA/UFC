# Prove migration 029 on the production engine with zero side effects:
#   BEGIN; <029 body>; <029 behavioural tests>; -- the tests always raise
# The final RAISE aborts the transaction, so nothing can commit whether the
# tests pass ('ALLPASS n') or fail ('FAIL ...').
#
#   pwsh scripts/db/prove_029.ps1
$ErrorActionPreference = "Stop"
$root = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
$sig = @'
using System; using System.Runtime.InteropServices;
public class CredMan029 {
...
'@
if (-not ([System.Management.Automation.PSTypeName]'CredMan029').Type) { Add-Type -TypeDefinition $sig }
$tok = [CredMan029]::Read("Supabase CLI:supabase")
if (-not $tok) { throw "Supabase CLI token not found" }
$mig = Get-Content (Join-Path $root "migrations/029_ufc_event_card_observations.sql") -Raw -Encoding UTF8
$mig = $mig -replace '(?im)^\s*begin;\s*$', '' -replace '(?im)^\s*commit;\s*$', ''
$tests = Get-Content (Join-Path $root "migrations/tests/029_ufc_event_card_observations.test.sql") -Raw -Encoding UTF8
...
try {
  ...
  if ($msg -match 'ALLPASS (\d+)') { "029 behavioural proof: ALL PASS ($($Matches[1]) checks); transaction rolled back" } else { "029 behavioural proof FAILED: $msg" }
}
