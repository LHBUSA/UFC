# Prove migration 027 on the production engine with zero side effects:
#   BEGIN; <027 body>; <027 behavioural tests>; -- the tests always raise
# The final RAISE aborts the transaction, so nothing can commit whether the
# tests pass ('ALLPASS n') or fail ('FAIL ...').
#
#   pwsh scripts/db/prove_027.ps1
$ErrorActionPreference = "Stop"
$root = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
$sig = @'
using System; using System.Runtime.InteropServices;
public class CredMan027 {
...
'@
if (-not ([System.Management.Automation.PSTypeName]'CredMan027').Type) { Add-Type -TypeDefinition $sig }
$tok = [CredMan027]::Read("Supabase CLI:supabase")
if (-not $tok) { throw "Supabase CLI token not found" }
$mig = Get-Content (Join-Path $root "migrations/027_ufc_algo_learning.sql") -Raw -Encoding UTF8
$mig = $mig -replace '(?im)^\s*begin;\s*$', '' -replace '(?im)^\s*commit;\s*$', ''
$tests = Get-Content (Join-Path $root "migrations/tests/027_ufc_algo_learning.test.sql") -Raw -Encoding UTF8
...
try {
  ...
  if ($msg -match 'ALLPASS (\d+)') { "027 behavioural proof: ALL PASS ($($Matches[1]) checks); transaction rolled back" } else { "027 behavioural proof FAILED: $msg" }
}
